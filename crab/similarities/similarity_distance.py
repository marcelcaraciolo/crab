...
def sim_euclidian(vector1,vector2,**args):
	...
	if type(vector1) == type({}):
		...
		sum_of_squares = sum([pow(vector1[i]-vector2[i],2.0) for i in range(len(vector1))])
...
def sim_pearson(vector1,vector2,**args):
	...
	if type(vector1) == type({}):
		...
		den = sqrt((sum1Sq-pow(sum1,2.0)/n) * (sum2Sq-pow(sum2,2.0)/n))
		...
		den = sqrt((sum1q-pow(sum1,2)/len(vector1))*(sum2q-pow(sum2,2)/len(vector1)))
...
def sim_cosine(vector1,vector2,**args):
	...
	if type(vector1) == type({}):		
		try:
			...
			v = [ (vector1[item], vector2[item]) for item in vector1 if item in vector2]
			...
			def norm(p):
				return sqrt(sum([p.get(item,0)* p.get(item,0)  for item in p]))
...
def sim_manhattan(vector1,vector2,**args):
	...
	if type(vector1) == type({}):
		nP1P2 =  len([ item  for item in vector1 if item in vector2 ])
		distance = sum([ abs(vector1[key] - vector2[key])   for key in vector1 if key in vector2 ])
	
	else:
		nP1P2 = len(vector1)
		distance = sum([ abs(vector1[i] - vector2[i])   for i in xrange(len(vector1))])
...
def sim_jaccard(vector1,vector2,**args):
	"""
	Jaccard similarity coefficient is a statistic used for comparing the similarity and diversity of sample sets.
	...
	if len(vector1) ==  0 and len(vector2) == 0: return 0.0
