...
================================================================
    ...
    This module contains basic implementations that encapsulate retrieval-related
    statistics about the quality of the recommender's recommendations.
...
class AverageAbsoluteDifferenceRecommenderEvaluator(RecommenderEvaluator):
    '''
    A Recommender Evaluator which computes the average absolute difference between predicted and actual ratings for users.
    '''
    ...
    def evaluate(self,recommender,dataModel,trainingPercentage,evaluationPercentage):
        ...
        for userID in dataModel.UserIDs():
            if random() < evaluationPercentage:
                self.processOneUser(trainingPercentage,trainingUsers,testUserPrefs,userID,dataModel)
    ...
    def processOneUser(self,trainingPercentage,trainingUsers,testUserPrefs,userID,dataModel):
        trainingPrefs = []
        testPrefs = []
    ...
    def getEvaluation(self,testUserPrefs,recommender):
        for userID,prefs in testUserPrefs.iteritems():
            ...
            for pref in prefs:
                try:
                    estimatedPreference = recommender.estimatePreference(userID=userID,itemID=pref,similarity=recommender.similarity)
                except:
                    #It is possible that an item exists in the test data but not training data in which case an exception will be throw.
                    #Just ignore it and move on.
                    pass
                if estimatedPreference is not None:
                    estimatedPreference = self.capEstimatePreference(estimatedPreference)
...
class RMSRecommenderEvaluator(AverageAbsoluteDifferenceRecommenderEvaluator):
    '''
    A Recommender Evaluator which computes the root mean squared difference between predicted and actual ratings for users. This is the 
    square root of the average of this difference, squared.
...
class IRStatsRecommenderEvaluator(RecommenderEvaluator):
    """
    For each user, this evaluator determine the top n preferences, then evaluate the IR statistics
    based on a DataModel that does not have these values. This number n is the 'at' value, as in
    'precision at 5'.  For example this would mean precision evaluated by removing the top 5 preferences
    for a user and then finding the percentage of those 5 items included in the top 5 recommendations for that user.
    """
    
    def evaluate(self,recommender,dataModel,at,evaluationPercentage,relevanceThreshold=None):
        if evaluationPercentage > 1.0 or evaluationPercentage < 0.0:
            raise Exception('Evaluation Percentage is above/under the limit.')
        if at < 1:
        ...
        for userID in dataModel.UserIDs():
            if random() < evaluationPercentage:
                ...
                for otherUserID in dataModel.UserIDs():
                    self.processOtherUser(userID,relevantItemIDs,trainingUsers,otherUserID,dataModel)
    ...
    def processOtherUser(self,userID,relevantItemIDs,trainingUsers,otherUserID,dataModel):
